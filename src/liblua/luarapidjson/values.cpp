...
namespace values {
	...
	namespace details {
		...
		static Value ArrayValue(lua_State* L, int idx, int depth, Allocator& allocator, int max);
		...
		Value TableValue(lua_State* L, int idx, int depth, Allocator& allocator)
		{
			...
			size_t max;
			return isarray(L, idx, &max) ? ArrayValue(L, idx, depth, allocator, (int)max) : ObjectValue(L, idx, depth, allocator);
		}
...
//						// [table, key, value, key]
//						lua_pushvalue(L, -2);
//						Value str = StringValue(L, -1, allocator);
//						lua_pop(L, 1);
//						// [table, key, value]
//						object.AddMember(str, toValue(L, -1, depth, allocator), allocator);
//						break;
//					}
//					default:{
//						break;
...
//				}
		...
		Value ArrayValue(lua_State* L, int idx, int depth, Allocator& allocator, int max)
		{
