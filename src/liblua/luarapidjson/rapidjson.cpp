...
private:
	...
	void encodeTable(lua_State* L, Writer* writer, int idx, int depth)
	{
		...
		lua_pushvalue(L, idx); // [table]
		size_t max;
		if (values::isarray(L, -1, &max))
		{
			encodeArray(L, writer, depth, (int)max);
			lua_pop(L, 1); // []
			return;
		}
...
//			}
	...
	template<typename Writer>
	void encodeArray(lua_State* L, Writer* writer, int depth, int max)
	{
//		auto table_index = lua_gettop(L);
//		auto element_index = table_index + 1;
		// [table]
		writer->StartArray();
...
//			lua_rawgeti(L, table_index, n); // [table, element]
//			lua_pushnumber(L, n);
//			lua_rawget(L, -2);
//			encodeValue(L, writer, -1, depth);
//			lua_pop(L, 1); // [table]
...
//	luax::setfuncs(L, methods); // [rapidjson]
	...
	luaL_openlib(L, "rapidjson", methods, 0);
